...
using C = onnx.onnx_cpp2py_export.version_converter;
...
public static class version_converter {
    
    static version_converter() {
...
";
    ...
    public static void convert_version(object model, object target_version) {
        // type: (ModelProto, int) -> ModelProto
        if (!(model is ModelProto)) {
            throw new ValueError("VersionConverter only accepts ModelProto as model, incorrect type: {}".format(type(model)));
        }
        if (!(target_version is int)) {
            throw new ValueError("VersionConverter only accepts int as target_version, incorrect type: {}".format(type(target_version)));
        }
    ...
    public static object ConvertError = C.ConvertError;
}
